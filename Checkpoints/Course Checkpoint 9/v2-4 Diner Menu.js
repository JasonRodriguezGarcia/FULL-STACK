...
//
// v2-4
// Added jump line in main/sides parsing
// Added price in main/sides parsing
var menus = [
...
* @param {array} arrayMainOrSides                  - array with main or sides for string generation
* @param {array} arrayMainOrSidesPrices            - array with main or sides prices for string generation
* @param {boolean} index                - index to add or not on string generation
*/
const stringOptionsGenerator = (arrayMainOrSides, arrayMainOrSidesPrices, index = false) => {
        const myArrayOptionsTextGenerator = [];
        for(var i=0; i<arrayMainOrSides.length; i++)
                myArrayOptionsTextGenerator.push(
                        `\n ${index?` ${i} -`:``} ${arrayMainOrSides[i]} ${arrayMainOrSidesPrices.length>0
                                ? `- `+(arrayMainOrSidesPrices[i]+(menuSelection.menuSelected == menus[1].menu[1]
...
const stringOptionsGeneratorWrapper = (typeSelected, selectMainOrSide) => {
        ...
        var keyMainOrSides = "";                        // Stores the main or side key in object menus
        ...
        if (selectMainOrSide == 0)
                ...
                keyMainOrSides = "sides";
        return stringOptionsGenerator(menus[arrayPosition][keyMainOrSides], [], false);        

// Old code to be deleted, yes ... I know it must to be deleted, but left to see where upper lines replace these lines
/*        if (typeSelected == menus[0].menu) // if breakfast
...
const stringCommentPrice = (typeSelected, menuMainSideChoice, mainOrSide) => {
        var arrayPosition = 0;                          // Stores breakfast/LunchDinner position in object menus
        var keyMainOrSidesComments = "";                // Stores the main or side comments key in object menus
        var keyMainOrSides = "";                        // Stores the main or side key in object menus
        var keyMainOrSidesPrices = "";                  // Stores the main or side prices key in object menus
        ...
        if (mainOrSide === "main"){
                keyMainOrSidesComments = "mainComments";
                keyMainOrSides = "main";
                keyMainOrSidesPrices = "mainPrices";
        } else {
                keyMainOrSidesComments = "sidesComments";
                keyMainOrSides = "sides";
                keyMainOrSidesPrices = "sidesPrices";
        }
        return `${menus[arrayPosition][keyMainOrSidesComments][menuMainSideChoice]} - ${menus[arrayPosition][keyMainOrSides][menuMainSideChoice]} - `+
        `${menus[arrayPosition][keyMainOrSidesPrices][menuMainSideChoice]+
...
const lineBillCreator = (typeSelected, mainOrSide, menuMainSideChoice) => {
    ...
    // depending from mainOrSide
        ...
        return `${menus[arrayPosition][keyMainOrSides][menuMainSideChoice]}:...  `+
        `${menus[arrayPosition][keyMainOrSidesPrices][menuMainSideChoice]+
                (menuSelection.menuSelected == menus[1].menu[1]
                ? dinnerAdditionalCost
                : 0)}$`;
...
//Menu type selection (Breakfast or LunchDinner), stored in menuSelection.menuSelected
...
// First parameter menuSelection.menuSelected (breakfast or lunch/diner)
...
do {
    menuSelection.mainSelected = prompt(`Please select Main:`+
                `${menuSelection.menuSelected == menus[0].menu // If = Breakfast
                        ? stringOptionsGenerator(menus[0].main, menus[0].mainPrices, true)
                        : stringOptionsGenerator(menus[1].main, menus[1].mainPrices, true)
                }`);
...
do {
        menuSelection.sideSelected = prompt(`Please select Side:`+
                `${menuSelection.menuSelected == menus[0].menu // If = Breakfast
                        ? stringOptionsGenerator(menus[0].sides, menus[0].sidesPrices, true)
                        : stringOptionsGenerator(menus[1].sides, menus[1].sidesPrices, true)
                }`);                
...
//
// Creating 1st line with 1st Item from arraySelection (menuType breakfast or lunchdinner)
