...
export default class BlogForm extends Component {
  constructor(props) {
    ...
    // DropZone component config settings
...
deleteImage(imageType) {
  axios
    .delete(
      `https://api.devcamp.space/portfolio/delete-portfolio-blog-image/${this.state
...
componentWillMount () {
  if (this.props.editMode) {
    this.setState ({
      // hacer un deconstruction más tarde
      id: this.props.blog.id,
      title: this.props.blog.title,
      blog_status: this.props.blog.blog_status,
      content: this.props.blog.content,
      apiUrl: `https://jasonrodriguez.devcamp.space/portfolio/`+
...
// calling axios as object BETTER
  axios({
    ...
    .then(response => {
      ...
      // reinitiating states
      // this line before next one to avoid warning in console
      this.setState({
        title: "",
        blog_status: "",
        content: "",
    ...
    .catch(error => {
      console.error("handleSubmit blog error***", error);
    });
    ...
    return (
      <form onSubmit={this.handleSubmit}
        ...
        <div className='one-column'>
          <RichTextEditor handleRichTextEditorChange=
                    {this.handleRichTextEditorChange}
